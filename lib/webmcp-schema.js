  ...
  async function callExecuteTool(context, tool, params) {
    ...
    if (tool && typeof tool.execute === 'function') {
      for (const style of order) {
        try {
          const arg = style === 'string' ? JSON.stringify(params) : params;
    ...
    if (context && typeof context.executeTool === 'function') {
      for (const style of order) {
        try {
          const arg = style === 'string' ? JSON.stringify(params) : params;
          const result = await context.executeTool(tool, arg);
      ...
      if (tool && typeof tool.name === 'string') {
        for (const style of order) {
          try {
            const arg = style === 'string' ? JSON.stringify(params) : params;
            const result = await context.executeTool(tool.name, arg);
            if (context) argumentStyles.set(context, style);
            return result;
          } catch (err) {
            const message = String((err && err.message) || err);
    ...
    if (context && typeof context.callTool === 'function' && tool && typeof tool.name === 'string') {
      return await context.callTool(tool.name, params);
    }
