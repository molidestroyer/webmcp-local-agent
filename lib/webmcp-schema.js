...
(function (root, factory) {
  ...
  async function callExecuteTool(context, tool, params) {
    // Try the form we learned first, but never treat it as settled: a cached
    // answer that later stops working used to throw raw, with no second
    // attempt, which is how a plain parse error reached the user again.
    const learned = argumentStyles.get(context);
    ...
    for (const style of order) {
      try {
        const result = await context.executeTool(
          tool,
          style === 'string' ? JSON.stringify(params) : params
        );
        argumentStyles.set(context, style);
        return result;
      } catch (err) {
        const message = String((err && err.message) || err);
        // Only an argument-parsing complaint justifies trying the other form.
        // Anything else is the tool's own failure and must not be replayed.
